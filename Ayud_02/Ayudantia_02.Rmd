...
Para los ejercicios a continuación es necesario que tengan instalados los siguientes paquetes, que cargaremos a continuación a esta sesión:w
...
```{r cbind}
dim(casos)[1]==dim(muertes)[1] # ver si tienen el mismo número de filas.# ---- DUDA: por qué es 1? qué representa? -------
...
```{r merge}
casosymuertes <- merge(casos,muertes, by = c("day","month","year","countriesAndTerritories"), all = T,sort=F) # ---- DUDA: si en by se pone los que tienen en común, por qué se ponen tan pocos? Cuál fue el críterio para elegir esas? RESPUESTA: Porque son las variables de la data muertes. 
...
# Cuantas filas con NAs hay, para que las cuente solo hay que agregar el ,.N 
...
```{r completa}
al_29_03_2020_completo <- al_29_03_2020[complete.cases(al_29_03_2020),] #-- DUDA: por qué aquí no usa el signo de exclamación? no se supone que eran las filas con NA?
```
